...
inline static uint4 set128(uint x, uint y, uint z, uint w) {
	return{ x, y, z, w };
}

static inline uint4 eq128(uint4 a, uint4 b) {
	return{ a.x == b.x ? (uint)-1 : 0, a.y == b.y ? (uint)-1 : 0, a.z == b.z ? (uint)-1 : 0, a.w == b.w ? (uint)-1 : 0 };
}
...
static inline uint4 add4(uint4 a, uint4 b) {
	return set128(a.x + b.x, a.y + b.y, a.z + b.z, a.w + b.w);
}
...
static inline uint4 add128(uint4 u, uint4 v) {
	...
	uint4 c2 = set128((c1.y | (c1.z&h.z))&h.y, c1.z&h.z, 0, 0); // Propagated carry
...
static inline uint4 inc128(uint4 u) {
	...
	uint4 c = set128(h.y&h.z&h.w & 1, h.z&h.w & 1, h.w & 1, 1);
	return add4(u, c);
}

// Return -U
static inline uint4 neg128(uint4 u) {
	return inc128(set128(u.x ^ (uint)-1, u.y ^ (uint)-1, u.z ^ (uint)-1, u.w ^ (uint)-1)); // (1 + ~U) is two's complement
}
